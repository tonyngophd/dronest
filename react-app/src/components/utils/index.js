...
export const convoKeyFromUserArray = arr => Array.from(new Set(arr.sort(sortThroughIds).map(el => JSON.stringify(el).replaceAll(':', '::'))));
export const userArrayFromConvoKey = key => Array.isArray(key) ? key.map(el => JSON.parse(el.replaceAll('::', ':'))) : key;
